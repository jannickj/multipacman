...
/**
 * @author Jan Misker
 * Wouter: WorldModel contains a "map" of the world. It can NOT draw itself, use a CaveView for that.
 * IMPORTANT: A square in the WorldModel is drawn ONLY if that square also contains GROUND.
 * I have no idea why Jan decided to do it this way. There is a simple test whether the agent is still
 * alive, agentIsAlive(), based on the board situation.
*/
...
public class WorldModel {
	...
	/** Check if the game has finished. Game finishes if agent climbs out of cave. Agent is dead if he is on top of
	 * living wumpus or in a pit. 
	 * @author W.Pasman
	*/
	...
	/* It's beyond me why Jan did not use a simple format like this for the files as well*/
    ...
    public WorldModel loadFrom(InputStream input) throws Exception {
        ...
        if (!fileInput.readUTF().equals(HEADER))
            throw new WumpusException("Invalid WorldModel-file");
