...
*/
	...
	public void Action(int pAction, WorldModel pWorld) {
		...
		// If agent has finished, the agent cannot do anything anymore.
		if(pWorld.gameFinished()) return;
		...
		if(pWorld.agentKilled()) score=score-1000; // just died...
	    calcPercepts(pWorld);
	}
	...
	*/
    private void calcPercepts(WorldModel pWorld) {
        ...
        if (pWorld.contains(agent, WorldModel.GOLD)) fPercept.setGlitter(true);
    }
