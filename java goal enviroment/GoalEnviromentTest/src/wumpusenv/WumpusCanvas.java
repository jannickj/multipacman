...
/**
 * This shows a canvas with the wumpus world on it. It needs a
 * WumpusCanvasListener object that maintains the Wumpus World model.
 */
class WumpusCanvas extends Canvas {
	
	private int zoom, xFit, yFit, squareSize;
	...
	public void update(Graphics g) {
		...
		if ((mouseStart != null) && (dragEnd != null)) {
			...
			offGraphics.drawRect(Math.min(dragStart.x, dragEnd.x), Math.min(
					dragStart.y, dragEnd.y), Math.max(dragEnd.x - dragStart.x,
					dragStart.x - dragEnd.x), Math.max(dragEnd.y - dragStart.y,
	...
	private void drawCoor(Graphics g) {
		...
		for (int y = focus.y - (yFit - 1) / 2; y < focus.y + (yFit + 1) / 2; y++) {
			g.drawString(String.valueOf(y), drawSpace.x - 5
					- fm.stringWidth(String.valueOf(y)), center.y
					- (y - focus.y) * squareSize - fm.getDescent());
		}
	}
